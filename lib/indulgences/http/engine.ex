defmodule Indulgences.Http.Engine do
  ...
  def execute(%Http{} = http, %{} = state) do
    ...
    {status, result} =
      case evaluted_http.method do
        :get! ->
          apply(HTTPoison, :get, [evaluted_http.url, evaluted_http.headers, evaluted_http.options])

        :post! ->
          apply(HTTPoison, :post, [
            evaluted_http.url,
            evaluted_http.body,
            evaluted_http.headers,
            evaluted_http.options
          ])
      end
      ...
      if http.check != nil do
        try do
          ...
          case Keyword.get(Function.info(http.check), :arity) do
            ...
            2 ->
              http.check.(result, state)

            _ ->
              raise "check function must accept one or two arguments"
          end
        catch
