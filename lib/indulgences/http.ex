defmodule Indulgences.Http do
  ...
  def is_status(%HTTPoison.Response{} = response, desired_status_code)
      when is_integer(desired_status_code) do
  ...
  def is_status(%HTTPoison.Response{} = response, desired_status_codes)
      when is_list(desired_status_codes) do
  ...
  defp update_header(%__MODULE__{} = http, key, value) do
    ...
    new_headers =
      case Enum.find_index(headers, fn {exist_key, _} -> exist_key == key end) do
